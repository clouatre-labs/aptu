...
### SARIF Content Policy

SARIF output generated by `aptu scan-security` contains only the following data:

- **File paths** (relative URI of the scanned file)
- **Line/region coordinates** (start line, start column, end line, end column)
- **Rule metadata** (short/full descriptions, remediation guidance, CWE/OWASP reference URLs)
- **Finding messages** (pattern-matched description, e.g., "Hardcoded API key detected")
- **Fingerprints** (SHA-256 hash for deduplication; does not include source content)

SARIF output does **not** include:

- Matched code snippets or source text
- Full-file content
- Secrets, credentials, or environment variable values

This minimal content policy ensures that uploading SARIF to GitHub Code Scanning does not expose source code beyond what is necessary to surface security findings.
