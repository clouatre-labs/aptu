...
6. Enforce prompt budget (`max_prompt_chars`): drop sections in order (`call_graph` -> `ast_context` -> `dep_enrichments` -> file patches -> file `full_content`) until budget is met
