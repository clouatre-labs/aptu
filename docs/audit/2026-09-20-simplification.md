...
6. **Decide on `--deep`** (#1651) — biggest remaining lever; gate on the existing `ReviewContextRecord` telemetry, pre-register the threshold, close with evidence either way (same methodology as the KG audits).
