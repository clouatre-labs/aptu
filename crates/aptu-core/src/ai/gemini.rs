...
use super::{AiResponse, GEMINI_API_KEY_ENV, GEMINI_API_URL};
...
impl GeminiClient {
        ...
        let completion: ChatCompletionResponse = (|| async {
            ...
            if !status.is_success() {
                if status.as_u16() == 401 {
                    anyhow::bail!(
                        "Invalid Gemini API key. Check your {GEMINI_API_KEY_ENV} environment variable."
                    );
                } else if status.as_u16() == 429 {
                    warn!("Rate limited by Gemini API");
    ...
    /// Analyzes a GitHub issue using the Gemini API.
    ///
    ...
    /// Creates a formatted GitHub issue using the Gemini API.
    ///
    ...
    pub async fn create_issue(
        ...
        debug!(model = %self.model, "Calling Gemini API for issue creation");
...
mod tests {
    ...
    fn test_build_create_system_prompt_contains_json_schema() {
        let prompt = build_create_system_prompt();
